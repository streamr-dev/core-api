...
public class StringStartsWith extends AbstractSignalPathModule {

	StringParameter s = new StringParameter(this,"search","");

	StringInput in = new StringInput(this,"text");

	TimeSeriesOutput out = new TimeSeriesOutput(this,"startsWith?");
	...
	public void sendOutput() {
		...
		if (!s.getValue().isEmpty()) {
			if ((ignoreCase && in.getValue().toLowerCase().startsWith(s.getValue().toLowerCase()))
				|| (!ignoreCase && in.getValue().startsWith(s.getValue())))
				out.send(1);
			else 
				out.send(0);
		}
