...
public class StringEndsWith extends AbstractSignalPathModule {

	StringParameter s = new StringParameter(this,"search","");

	StringInput in = new StringInput(this,"text");

	TimeSeriesOutput out = new TimeSeriesOutput(this,"endsWith?");
	...
	public void sendOutput() {
		...
		if (!s.getValue().isEmpty()) {
			if ((ignoreCase && in.getValue().toLowerCase().endsWith(s.getValue().toLowerCase()))
				|| (!ignoreCase && in.getValue().endsWith(s.getValue())))
				out.send(1);
			else 
				out.send(0);
		}
