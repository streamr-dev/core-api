...
consumer.on('message', function (message) {
	// Strip headers generated by UnifinaKafkaProducer
	message.value = message.value.substring(message.value.indexOf("{"))
	console.log("Emitting Kafka message: "+message.value)
		...
		if (data.channels) {
			...
			data.channels.forEach(function(channel) {
				...
				socket.join(channel, function(err) {
					...
					if (subCount===data.channels.length) {
						// Ack subscription
						socket.emit('subscribed', {channels:data.channels, error:err})						
					}
				})
				
				kafkaSubscribe(channel)
			})
