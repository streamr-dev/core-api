...
databaseChangeLog = {
	...
	changeSet(author: "kkn", id: "test-data-products-permissions", context: "test") {
		grailsChange {
			change {
				sql.eachRow("select id from product") { row ->
					...
					[
						Permission.Operation.PRODUCT_GET.id,
						Permission.Operation.PRODUCT_SHARE.id,
						Permission.Operation.PRODUCT_EDIT.id,
						Permission.Operation.PRODUCT_DELETE.id,
					].each { operation ->
