...
//			ssp.live = json.signalPathContext.live
	...
	def loadBrowserContent() {
		...
		if (params.browserId == 'examplesLoadBrowser') {
			ssp = SavedSignalPath.executeQuery("select sp.id, sp.name from SavedSignalPath sp where sp.type = :type order by sp.id desc", [type:SavedSignalPath.TYPE_EXAMPLE_SIGNAL_PATH], [max: max, offset: offset])
		} else {
