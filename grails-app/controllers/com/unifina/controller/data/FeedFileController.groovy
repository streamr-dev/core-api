...
class FeedFileController {
	...
	private static final Logger log = Logger.getLogger(FeedFileController.class)
	...
	def index() {
		if (grailsApplication.config.unifina.feed.datadir) {
			...
			// POST to save a file (very unsafe, use for environment mocking only)
				...
				FileChannel fileChannel = FileChannel.open(path, EnumSet.of(StandardOpenOption.WRITE, StandardOpenOption.CREATE_NEW))
