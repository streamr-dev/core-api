...
class SubscriptionServiceSpec extends BeanMockingSpecification {
	...
	void "onSubscribed() creates subscription-linked permissions if user found for address"() {
		...
		1 * permissionService.systemGrant(user, s1, Permission.Operation.STREAM_GET, _, _) >> p1
		1 * permissionService.systemGrant(user, s2, Permission.Operation.STREAM_GET, _, _) >> p2
	...
	void "onSubscribed() does not remove existing non-subscription permissions if user found for address"() {
		...
		then:
		1 * ethereumIntegrationKeyService.getEthereumUser(address) >> user
		1 * permissionService.systemGrant(user, s1, Permission.Operation.STREAM_GET, _, _) >> p3
		1 * permissionService.systemGrant(user, s2, Permission.Operation.STREAM_GET, _, _) >> p4
	...
	void "onSubscribed() removes existing subscription-linked permissions if user found for address"() {
		...
		Permission p1 = new Permission(user: user, stream: s1, operation: "stream_get")
		Permission p2 = new Permission(user: user, stream: s2, operation: "stream_get")
		p1.subscription = s
		p2.subscription = s
		...
		Permission p3 = new Permission(operation: "stream_get", user: user, stream: s1, endsAt: new Date())
		Permission p4 = new Permission(operation: "stream_get", user: user, stream: s2, endsAt: new Date())
	...
	void "subscribeToFreeProduct() creates new FreeSubscription if product-user pair does not exist"() {
		...
		Permission p1 = new Permission(operation: "stream_get", user: user, stream: s1)
		Permission p2 = new Permission(operation: "stream_get", user: user, stream: s2)
	...
	void "subscribeToFreeProduct() creates subscription-linked permissions"() {
		...
		Permission p1 = new Permission(operation: Permission.Operation.STREAM_GET, user: user, stream: s1).save(failOnError: true, validate: false)
		Permission p2 = new Permission(operation: Permission.Operation.STREAM_GET, user: user, stream: s2).save(failOnError: true, validate: false)
	...
	void "subscribeToFreeProduct() removes existing subscription-linked permissions"() {
		...
		Subscription s = new FreeSubscription(product: product, user: user2, endsAt: new Date(0))
			.save(failOnError: true, validate: false)
		Permission p1 = new Permission(user: user, stream: s1, operation: Permission.Operation.STREAM_GET)
		Permission p2 = new Permission(user: user, stream: s2, operation: Permission.Operation.STREAM_GET)
		p1.subscription = s
		p2.subscription = s
		p1.save(failOnError: true, validate: false)
		p2.save(failOnError: true, validate: false)
		...
		Permission p3 = new Permission(operation: Permission.Operation.STREAM_GET, user: user, stream: s1)
		Permission p4 = new Permission(operation: Permission.Operation.STREAM_GET, user: user, stream: s2)

		when:
		service.subscribeToFreeProduct(product, user2, new Date())

		then:
		1 * permissionService.systemGrant(user2, s1, Permission.Operation.STREAM_GET, _, _) >> p3
		1 * permissionService.systemGrant(user2, s2, Permission.Operation.STREAM_GET, _, _) >> p4
		1 * permissionService.systemRevoke(p1) >> { p1.delete(flush: true) }
		1 * permissionService.systemRevoke(p2) >> { p2.delete(flush: true) }
		!Permission.exists(p1.id)
		!Permission.exists(p2.id)
	...
	void "afterIntegrationKeyCreated() creates subscription-linked permissions for given integration key"() {
		...
		Permission p1 = new Permission(operation: Permission.Operation.STREAM_GET, user: user, stream: s1).save(failOnError: true, validate: false)
		Permission p2 = new Permission(operation: Permission.Operation.STREAM_GET, user: user, stream: s2).save(failOnError: true, validate: false)
		Permission p3 = new Permission(operation: Permission.Operation.STREAM_GET, user: user, stream: s3).save(failOnError: true, validate: false)
		...
		1 * permissionService.systemGrant(user, s1, Permission.Operation.STREAM_GET, _, _) >> p1
		1 * permissionService.systemGrant(user, s2, Permission.Operation.STREAM_GET, _, _) >> p2
		1 * permissionService.systemGrant(user, s3, Permission.Operation.STREAM_GET, _, _) >> p3
	...
	void "afterIntegrationKeyCreated() updates subscription-linked permissions on a per-user basis"() {
		...
		p1 = new Permission(user: user, stream: s1, operation: Permission.Operation.STREAM_GET, subscription: sub1, endsAt: date).save(failOnError: true, validate: false)
		p2 = new Permission(user: user, stream: s2, operation: Permission.Operation.STREAM_GET, subscription: sub1, endsAt: date).save(failOnError: true, validate: false)
		...
		p3 = new Permission(user: user2, stream: s3, operation: Permission.Operation.STREAM_GET, subscription: sub2, endsAt: date).save(failOnError: true, validate: false)
		...
		interaction {
			...
			1 * permissionService.systemGrant(user, s3, Permission.Operation.STREAM_GET, _, _) >> p1
			1 * permissionService.systemGrant(user2, s2, Permission.Operation.STREAM_GET, _, _) >> p4
		}
