...
class ProductSpec extends Specification {
	...
	@Unroll
	void "isUndeploying(#state) == #expected"(Product.State state, boolean expected) {
		expect:
		Product p = new Product(state: state)
		p.isUndeploying() == expected

		where:
		state | expected
		Product.State.NOT_DEPLOYED | false
		Product.State.DEPLOYING | false
		Product.State.UNDEPLOYING | true
		Product.State.DEPLOYED | false
	}

	@Unroll
	void "isNotDeployed(#state) == #expected"(Product.State state, boolean expected) {
		expect:
		Product p = new Product(state: state)
		p.isNotDeployed() == expected

		where:
		state | expected
		Product.State.NOT_DEPLOYED | true
		Product.State.DEPLOYING | false
		Product.State.UNDEPLOYING | false
		Product.State.DEPLOYED | false
	}
	...
	@Unroll
	void "isDeploying(#state) == #expected"(Product.State state, boolean expected) {
		expect:
		Product p = new Product(state: state)
		p.isDeploying() == expected

		where:
		state | expected
		Product.State.NOT_DEPLOYED | false
		Product.State.DEPLOYING | true
		Product.State.UNDEPLOYING | false
		Product.State.DEPLOYED | false
	}
}
