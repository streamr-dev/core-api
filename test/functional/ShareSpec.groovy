...
class ShareSpec extends LoginTester1Spec {
	...
	void "Stream list page: sharePopup rejects bad email, ESC discards changes"() {
		when:
		to StreamListPage
		then:
		!$(".bootbox.modal")
	...
	void "Stream list page: sharePopup adds permissions"() {
		when:
		to StreamListPage
		then:
		!$(".bootbox.modal")
	...
	void "Move to Stream page, revoke permission"() {
		when:
		to StreamListPage
		$(".tr", "data-id": "1").click()
		then:
